"use strict";

let running = false;

$(function () {
    let player_one, player_two, game;

    $.ajax({
        type: "get",
        url: "/groep-28/web/assets/json/keybinds.json",
        dataType: "json",
        success: function (res) {
            console.log(res);
            player_one = new Player("player_one", res.player_one);
            player_two = new Player("player_two", res.player_two);
        },
        error: function (res) {
            console.log(res);
        }
    });

    $(window).keydown(function (e) {
        let key = e.keyCode;

        if (player_one.keybinds.includes(key)) {
            player_one.move(key);
        } else if (player_two.keybinds.includes(key)) {
            player_two.move(key);
        }
    });

    $('#start').on('click', (e) => {
        game = new Game(player_one, player_two);
        game.start();
})
    $('#pause').on('click', (e)=>{
        $('.modal_container';).fadeIn();
})
    $('#close').on('click', (e)=>{
        $('.modal_container';).fadeOut();
    })
});

function Game(player_one, player_two) {
    let gameloop;

    let ResetGame = () =;> {
        player_one.reset();
        player_two.reset();
    }

    let update = () =;> {
        let po = player_one.update();
        let pt = player_two.update();
        console.log(po + " " + pt);

        if(!pt && pt != null || !po && po != null){
            console.log("end");
            endGame();
            running = false;
        }
    }

    let endGame = () =;> {
        running = false;
        clearInterval(gameloop);

        player_one.gameOver();
        player_two.gameOver();
    }

    this.start = () =;> {
        if (!running) {
            running = true;
            ResetGame();
        }

        gameloop = setInterval(function () {
            if (running) {
                update();
            } else {
                endGame();
            }
        }, 10);
        this.disabled = true;
    }
}

function Player(p, JSONkeys) {
    let setKeybinds = (keybindsJSON) =;> {
        this.up = keybindsJSON.up;
        this.down = keybindsJSON.down;
        this.left = keybindsJSON.left;
        this.right = keybindsJSON.right;
        this.heropower = keybindsJSON.heropower;

        this.keybinds = [this.up, this.down, this.left, this.right, this.heropower];
    }

    let canvas = $('#' + p + ' .field')[0];
    let context = canvas.getContext("2d");
    context.scale(20, 20);

    let makeMatrix = function (w, h) {
        const matrix = [];
        while (h--) {
            matrix.push(new Array(w).fill(0));
        }
        return matrix;
    };

    let makePiece = function (type) {
        if (type === "t") {
            return [
                [0, 0, 0],
                [5, 5, 5],
                [0, 5, 0]
            ];
        } else if (type === "o") {
            return [
                [7, 7],
                [7, 7]
            ];
        } else if (type === "l") {
            return [
                [0, 4, 0],
                [0, 4, 0],
                [0, 4, 4]
            ];
        } else if (type === "j") {
            return [
                [0, 1, 0],
                [0, 1, 0],
                [1, 1, 0]
            ];
        } else if (type === "i") {
            return [
                [0, 2, 0, 0],
                [0, 2, 0, 0],
                [0, 2, 0, 0],
                [0, 2, 0, 0]
            ];
        } else if (type === "s") {
            return [
                [0, 3, 3],
                [3, 3, 0],
                [0, 0, 0]
            ];
        } else if (type === "z") {
            return [
                [6, 6, 0],
                [0, 6, 6],
                [0, 0, 0]
            ];
        }
    };

    let points = function () {
        let rowCount = 1;
        outer: for (let y = area.length - 1; y > 0; --y) {
            for (let x = 0; x < area[y].length; ++x) {
                if (area[y][x] === 0) {
                    continue outer;
                }
            }
            const row = area.splice(y, 1)[0].fill(0);
            area.unshift(row);
            ++y;
            player.score += rowCount * 100;
            rowCount *= 2;
        }
    };
    let collide = function (area, player) {
        const [m, o] = [player.matrix, player.pos];
        for (let y = 0; y < m.length; ++y) {
            for (let x = 0; x < m[y].length; ++x) {
                if (m[y][x] !== 0 && (area[y + o.y] && area[y + o.y][x + o.x]) !== 0) {
                    return true;
                }
            }
        }
        return false;
    };
    let drawMatrix = function (matrix, offset) {
        matrix.forEach((row, y) => {
            row.forEach((value, x) => {
                if (value !== 0;) {
                    // context.fillStyle=colors[value];
                    // context.fillRect(x+offset.x,y+offset.y,1,1);
                    let imgTag = document.createElement("IMG");
                    imgTag.src = colors[value];
                    context.drawImage(imgTag, x + offset.x, y + offset.y, 1, 1);
                }
    })
    })
    };
    let merge = function (area, player) {
        player.matrix.forEach((row, y) => {
            row.forEach((value, x) => {
                if (value !== 0;) {
                    area[y + player.pos.y][x + player.pos.x] = value;
                }
    })
    })
    };
    let rotate = function (matrix, dir) {
        for (let y = 0; y < matrix.length; ++y) {
            for (let x = 0; x < y; ++x) {
                [
                    matrix[x][y],
                    matrix[y][x]
                ] = [
                    matrix[y][x],
                    matrix[x][y],
                ]
            }
        }
        if (dir > 0) {
            matrix.forEach(row = > row.reverse();
        )
        } else {
            matrix.reverse();
        }
    };

    let playerReset = function () {
        //ijlostz
        const pieces = "ijlostz";
        player.matrix = makePiece(pieces[Math.floor(Math.random() * pieces.length)]);
        player.pos.y = 0;
        player.pos.x = (Math.floor(area[0].length / 2)) - (Math.floor(player.matrix[0].length / 2));
        if (collide(area, player)) {
            area.forEach(row = > row.fill(0);
        )
            player.score = 0;

            return false;
        }

        return true;
    };

    let playerDrop = function () {
        player.pos.y++;
        player.score += 10;
        if (collide(area, player)) {
            player.pos.y--;
            merge(area, player);
            points();
            updateScore();

            return playerReset();
        }
    };
    let playerMove = function (dir) {
        player.pos.x += dir;
        if (collide(area, player)) {
            player.pos.x -= dir;
        }
    };

    let playerRotate = function (dir) {
        const pos = player.pos.x;
        let offset = 1;
        rotate(player.matrix, dir);
        while (collide(area, player)) {
            player.pos.x += offset;
            offset = -(offset + (offset > 0 ? 1 : -1));
            if (offset > player.matrix[0].length) {
                rotate(player.matrix, -dir);
                player.pos.x = pos;
                return;
            }
        }
    };

    let draw = function () {
        context.clearRect(0, 0, canvas.width, canvas.height);
        context.fillStyle = "rgba(0,0,0,0)";
        context.fillRect(0, 0, canvas.width, canvas.height);
        updateScore();
        drawMatrix(area, {
            x: 0,
            y: 0
        });
        drawMatrix(player.matrix, player.pos);
    };
    let dropInter = 100;
    let time = 0;

    let updateScore = function () {
        context.font = "bold 1px Comic Sans MS";
        context.fillStyle = "#ffffff";
        context.textAlign = "left";
        context.textBaseline = "top";
        context.fillText("Score:" + player.score, 0.2, 0);
    };

    const colors = [
        null,
        "data:image/png;base64,",
        "data:image/png;base64,",
        "data:image/png;base64,",
        "data:image/png;base64,",
        "data:image/png;base64,",
        "data:image/png;base64,",
        "data:image/png;base64,"
    ];

    const area = makeMatrix(12, 20);
    const player = {
        pos: {
            x: 0,
            y: 0
        },
        matrix: null,
        score: 0
    };
    const move = 1;

    this.playingfield = $('#' + p + " .field")[0];

    this.move = (key) =;> {
        switch (key) {
            case this.up:
                playerRotate(-move);
                break;
            case this.down:
                playerDrop();
                break;
            case this.right:
                playerMove(+move);
                break;
            case this.left:
                playerMove(-move);
                break;
                /* add one more for hero power */
        }
    }
    this.reset = function () {
        return playerReset();
    };

    this.update = function () {
        time++;
        let bool;
        if (time >= dropInter) {
            bool = playerDrop();
            time = 0;
        }
        draw();

        return bool;
    };

    this.gameOver = function () {
        context.font = "2px Comic Sans MS";
        context.fillStyle = "#ffffff";
        context.textAlign = "center";
        context.textBaseline = "middle";
        context.fillText("Game Over", (canvas.width / 20) / 2, (canvas.width / 20) / 2);
        document.getElementById("start_game").disabled = false;
    };

    setKeybinds(JSONkeys);
    playerReset();
    draw();
}